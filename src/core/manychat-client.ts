...
export const DEFAULT_MANYCHAT_API_BASE_URL = "https://api.manychat.com/fb";
...
export class ManyChatClient {
  ...
  constructor(apiKeyOrOptions?: string | ManyChatClientOptions) {
    const options =
      typeof apiKeyOrOptions === "string"
        ? { apiKey: apiKeyOrOptions }
        : apiKeyOrOptions ?? {};
    const key = options.apiKey ?? process.env.MANYCHAT_API_KEY;
    if (!key) {
      throw new Error(
    ...
    this.apiKey = key;
    this.baseUrl =
      options.baseUrl ?? process.env.MANYCHAT_API_BASE_URL ?? DEFAULT_MANYCHAT_API_BASE_URL;
  }

...
export function registerPolicyTools(
  ...
  server.registerTool("validate_message", { description: "Validate an outbound ManyChat message against Meta policy (24h window, message tags, HUMAN_AGENT, opt-in) BEFORE sending. Returns allowed=false when a send would violate policy.", inputSchema: z.object({
              channel: z
